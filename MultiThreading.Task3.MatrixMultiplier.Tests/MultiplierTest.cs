    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            while (true)
            {
                ...
                watch1.Stop();
                //Debug.WriteLine($"Regular multiply for {watch1.ElapsedMilliseconds} msecs");
                var regTime = watch1.ElapsedMilliseconds;

                watch1.Restart();
                IMatrix resultMatrix2 = new MatricesMultiplierParallel().Multiply(firstMatrix, secondMatrix);
                watch1.Stop();
                //Debug.WriteLine($"Parallel multiply for {watch1.ElapsedMilliseconds} msecs");
                var parTime = watch1.ElapsedMilliseconds;

                if(regTime > parTime)
				{
                    Debug.WriteLine($"With matrixSize = {matrixSize} Parallel multiply for {parTime} ms faster then Regular multiply for {regTime} ms");
                    break;
                }
                matrixSize++;
            }
